...
score_theory     = ROOT.TF1("score_theory", "1./{sigma}*( ((x-{mu})/{sigma})**2 - 1)".format(mu=mu, sigma=sigma), xmin, xmax)
...
texX   = "x"
weighted      = False
id_string     = "nTrees%i-gaussian_width"%(n_trees)
...
def get_sampled_dataset( n_events ):
    ...
    diff_weights  = np.array( [ 1./sigma*( ((features[i][0]-mu)/sigma)**2 -1 ) for i in range(n_events)] )

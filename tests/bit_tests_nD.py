...
argParser.add_argument("--model",              action="store",      default="exponential_nD", type=str,   choices=allModels,                    help="import model")
...
argParser.add_argument("--ndim",               action="store",      default=1,             type=int,                                         help="number of dimension")
...
# directory for plots
plot_directory = os.path.join( user_plot_directory, args.plot_directory, str(args.ndim)+"D", model.id_string )
...
training_features, training_weights, training_diff_weights = model.get_dataset( args.nTraining, args.ndim )
...
# sum x1,x2,x3 to x = sqrt(x1^2 + .. + xD^2)
...
# subtitude x with sqrt(x1^2 + ... + xD^2)
...
for i in range(args.nTraining):
    h_SM.Fill ( sumFeatures(training_features[i]), training_weights[i] )
    h_BSM.Fill( sumFeatures(training_features[i]), training_weights[i]+args.theta*training_diff_weights[i] )
...
# Testing
test_features, test_weights, test_diff_weights = model.get_dataset( args.nTraining, args.ndim )
...
def score_histo( bit, title, max_n_tree = None):
    h = ROOT.TH1F(str(title), str(title), 40, model.xmin, model.xmax)
    for i in range(1, h.GetNbinsX()+1):
        h.SetBinContent( i, bit.predict(getSubstitude(h.GetBinLowEdge(i), args.ndim), max_n_tree = max_n_tree, last_tree_counts_full=False))
...
outfile = ROOT.TFile(plot_directory+'/score_boosted.root', 'recreate')
outfile.cd()
histos[-1][0].Write()
outfile.Close()

...
ROOT.gROOT.LoadMacro(os.path.join( dir_path, "scripts/tdrstyle.C"))
ROOT.setTDRStyle()
...
argParser.add_argument("--plot_directory",     action="store",      default="multiBIT",      help="plot sub-directory")
...
argParser.add_argument("--coefficients",       action="store",      default=['cHW', 'cHWtil', 'cHQ3'],   nargs="*", help="Which coefficients?")
...
# We only parse the known arguments. The ones we don't know are interpreted as BIT configuration options. Run with, e.g., "--min_size 1000 --prefix minSize1k"
...
if not os.path.exists( user.model_directory ):
    try:
        os.makedirs( user.model_directory )
    except IOError:
        pass
...
try:
    ...
    bit = None
if bit is None:
    print ("Loading unsuccessful. We'll train.")
else:
    print ("Loading successful.")

# reweight training data according to bias. Use, e.g., as  --bias "pT" "10**(({}-200)/200"

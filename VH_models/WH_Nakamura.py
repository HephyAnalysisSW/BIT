...
def getWeights(features, eft):
    ...
    w           = (s_hat + m['W']**2-m['H']**2)/(2*np.sqrt(s_hat))
    k           = np.sqrt( w**2-m['W']**2 )
    ...
    kappa_WW       = 2*v**2/eft['Lambda']**2*eft['cHW']
    kappa_WW_tilde = 2*v**2/eft['Lambda']**2*eft['cHWtil']
    g_h_w_Q        = sqrt_2*g*v**2/eft['Lambda']**2*eft['cHQ3'] 

    for lambda_boson in [+1, -1, 0]:
        if abs(lambda_boson)==1:
            prefac = g**2/sqrt_2*m['W']*sqrt_s_hat/(s_hat-m['W']**2)
            M    = {tuple()   :   prefac*(1. + 0.5*kappa_WW*(1+(s_hat-m['H']**2)/m['W']**2) - 1j*lambda_boson*kappa_WW_tilde*k*sqrt_s_hat/m['W']**2 ) + 0.5*g*g_h_w_Q*sqrt_s_hat/m['W'],
                   ('cHW',)   :   prefac*0.5*(1+(s_hat-m['H']**2)/m['W']**2)*2*v**2/eft['Lambda']**2,
                   ('cHWtil',):   prefac*( -1j*lambda_boson*k*sqrt_s_hat/m['W']**2*2*v**2/eft['Lambda']**2),
                   ('cHQ3',)  :   0.5*g*sqrt_s_hat/m['W']*sqrt_2*g*v**2/eft['Lambda']**2,
                   }
            ...
            prefac = g**2/sqrt_2*w*sqrt_s_hat/(s_hat-m['W']**2) 
            M = {tuple() :     -prefac*(1+0.5*kappa_WW*(1+(s_hat-m['H']**2)/m['W']**2 - 2*k**2*sqrt_s_hat/(m['W']**2*w))) - g*g_h_w_Q*w*sqrt_s_hat/(2*m['W']**2), 
                ('cHW',) :     -prefac*0.5*(1+(s_hat-m['H']**2)/m['W']**2 - 2*k**2*sqrt_s_hat/(m['W']**2*w))*2*v**2/eft['Lambda']**2,
                ('cHWtil',):   np.zeros(N_events),
                ('cHQ3',):     -g*w*sqrt_s_hat/(2*m['W']**2)*sqrt_2*g*v**2/eft['Lambda']**2,
                    }

            M_lambda_udbar[lambda_boson] = { k: -sin_theta*M[k] for k in M.keys()} 
            M_lambda_dbaru[lambda_boson] = { k: M_lambda_udbar[lambda_boson][k] for k in M.keys()}

            M_lambda_dubar[lambda_boson] = { k: -sin_theta*M[k] for k in M.keys()}
    ...
    dsigmaWH_prefac = m['W']*k/(constWH*s_hat**1.5)*g**2*2 #factor 2 from sum_{u,d} |V_{ud}|**2 = 2

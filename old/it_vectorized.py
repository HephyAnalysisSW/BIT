...
if __name__=="__main__":
    ...
    for split_method in ['python_loop', 'vectorized_split_and_weight_sums']:
        tic  = time.time()
        node = Node( features, weights, FI_func=FI_func, max_depth=max_depth, min_size=min_size, weight_mask=weight_mask, diff_weight_mask=diff_weight_mask, split_method=split_method )
        toc = time.time()
        print("tree construction in {time:0.4f} seconds for method {method:s}".format(time=toc-tic, method=split_method))

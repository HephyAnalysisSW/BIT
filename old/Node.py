...
class Node:
    def __init__( self, features, max_depth, min_size, training_weights, training_diff_weights, split_method="vectorized_split_and_weight_sums", _depth=0, max_uncertainty=0):
        ...
        #assert self.max_uncertainty==0 or split_method == "iterative_split_and_weight_sums", "max_uncertainty argument only for split_method=iterative_split_and_weight_sums"
...
#    def get_split_vectorized_old( self ):
...
#        return argmax_fi, fisher_gains[argmax_fi]
    ...
    def get_split_iterative( self ):
        ...
        for i_feature in range(len(self.features[0])):
            ...
            diff_weight_sum_low     = np.sum(self.training_diff_weights[args_sorted[:self.min_size]])

            # special case: 2x min_size, i.e. nothing to look for
            if self.size == 2*self.min_size:
                gain = 0
                if weight_sum_low>0:
                    gain+= diff_weight_sum_low**2/weight_sum_low
                if weight_sum - weight_sum_low>0:
                    gain+= ( diff_weight_sum - diff_weight_sum_low)**2/(weight_sum-weight_sum_low)
                if gain > self.split_gain: 
                    self.split_i_feature = i_feature
            ...
            while True:
                ...
                if self.max_uncertainty>0:
                    ...
                    for i_quantile, quantile in reversed(list(enumerate(quantiles))):
                        ...
                        if not max_jackknive_rel_uncertainty<=self.max_uncertainty:
                ...
                diff_weight_sums = diff_weight_sum_low + np.cumsum( [ np.sum(training_diff_weights[args_sorted][digitized_features==i]) for i in range( n_threshold  +1) ] )

                #print "weight_sum", weight_sum
                #print "diff_weight_sum", diff_weight_sum
                #print "weight_sums", weight_sums
                #print "diff_weight_sums", diff_weight_sums

                fisher_information_left  = np.divide( diff_weight_sums*diff_weight_sums, weight_sums, out=np.zeros_like(diff_weight_sums), where=weight_sums!=0) 
                diff_weight_sums_right   = diff_weight_sum-diff_weight_sums
                weight_sums_right        = weight_sum-weight_sums
                fisher_information_right = np.divide( diff_weight_sums_right*diff_weight_sums_right, weight_sums_right, out=np.zeros_like(diff_weight_sums_right), where=weight_sums_right!=0) 
                ...
                diff_weight_sum_low += np.sum(training_diff_weights[args_sorted[:remove_low]])
...
#        elif self.split_method == "vectorized_split_and_weight_sums_old":
#            self.get_split_vectorized_old()
